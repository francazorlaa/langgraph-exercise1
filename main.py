...
async def get_universities():
    try:
        ...
        return {"universities": universities, "message": "Universidades obtenidas exitosamente"}
    except Exception as e:
        logger.error(f"Error al obtener universidades: {e}")
        raise HTTPException(status_code=500, detail="Error al obtener universidades")

# Ruta para obtener una universidad por nombre (solo la primera que coincida)
@app.get("/universities/{name}")
async def get_one_university(name: str):
    try:
        university = db["universities"].find_one({"name": name})
        if university:
            university["_id"] = str(university["_id"])
            logger.info(f"Universidad {name} obtenida exitosamente")
            return {"university": university, "message": f"Universidad {name} obtenida exitosamente"}
        else:
            raise HTTPException(status_code=404, detail="Universidad no encontrada")
    except Exception as e:
        logger.error(f"Error al obtener universidad: {e}")
        raise HTTPException(status_code=500, detail="Error al obtener universidad")
...
async def update_university(university_id: str, university: University):
    try:
        ...
        if result.matched_count == 0:
            raise HTTPException(status_code=404, detail="Universidad no encontrada")
        logger.info(f"Universidad con ID {university_id} actualizada exitosamente")
        return {"message": f"Universidad con ID {university_id} actualizada exitosamente"}
    except Exception as e:
        logger.error(f"Error al actualizar universidad: {e}")
        raise HTTPException(status_code=500, detail="Error al actualizar universidad")
